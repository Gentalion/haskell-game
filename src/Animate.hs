...
animateBattle f b = case (movingSquad b, movingEnemies b) of
    (Just ms,_) -> 
        ...
        in case (MovingSquad.animation ms, control squad) of
            ...
            ([], Enemy) -> return $ checkEnemyTargets (modifyBattleWithCell cell (b {movingSquad = Nothing}))
            ([],     _) -> return $ b {movingSquad = Nothing}
            ( _,     _) -> return $ b {movingSquad = Just $ MovingSquad.animateMovingSquad f ms}
    (Nothing,[]) -> return $ b
    (Nothing, l) -> return $ enemyAIturn b