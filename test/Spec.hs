...
main = 
    let f = generateHexField 6 5
        ...
        b3 = modifyBattleWithCell (c {position = (3,2), terrain = TerPlain, squad = Just (def {control = EnemyAI, rotation = 0.0, units = take 9 $ repeat def})}) b2
        b4 = modifyBattleWithCell (c {position = (3,3), terrain = TerPlain, squad = Just (def {control = EnemyAI, rotation = 0.0, units = take 9 $ repeat def})}) b3
