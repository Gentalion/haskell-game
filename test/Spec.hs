...
main :: IO ()
main = 
    let f = generateHexField 5 5
        b = Battle {field = f, fieldHeight = 5, fieldWidth = 5, allies = [], enemies = [], enemiesRemaining = 0, selection = Nothing, possibleMoves = [], previousTurns = []}
        c = Cell {position = (0,0), terrain = TerPlain, squad = Nothing}
        b1 = modifyBattleWithCell b (c {position = (1,1), terrain = TerWater})
