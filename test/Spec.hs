...
main = 
    let f = generateHexField 5 9
        ...
        b1 = modifyBattleWithCell (c {position = (1,1), terrain = TerPlain, squad = Just (emptySquad {control = Player, rotation = 0.0, units = take 7 (repeat emptyUnit)})}) b
        b2 = modifyBattleWithCell (c {position = (2,2), terrain = TerPlain, squad = Just (emptySquad {control = EnemyAI, rotation = 0.0, units = take 9 (repeat emptyUnit)})}) b1
    ...
    --in drawGame b3
    in interactIO greyN b3 drawBattlePicture managePlayerInput